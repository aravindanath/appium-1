...
iOSController.getScreenshot = function (customScreenshotFolder, cb) {
  var guid = new Date().getTime() + "_" + uuid.create();
  var command = ["au.capture('", guid, "');"].join('');
  ...
  var shotName = [guid, ".png"].join("");
  var shotPath = [screenshotFolder, shotName].join("");
  ...
  async.retry(3,
    function (cb) {
      async.waterfall([
        ...
        function (response, data, cb) {
          if (typeof customScreenshotFolder !== "undefined" && customScreenshotFolder !== null){
            fs.readFile(shotPath, function (err, content){
              if (err) return cb(new Error("Could not read file from temp folder. " + err.toString()));
              else { 
                fs.writeFile(customScreenshotFolder + "/" + shotName, content, function (err){
                  if (err) return cb(new Error("Could not write screenshot in custom folder. " + err.toString()), null);
            	    else {
