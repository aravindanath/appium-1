...
androidCommon.getScreenshot = function(screenshotDir, cb) {
  var localfile;
  if (screenshotDir !== null){
    localfile = [screenshotDir, '/', new Date().getTime(), '_', uuid.create() + '.png'].join('');
  } else {
    localfile = temp.path({prefix: 'appium', suffix: '.png'});
  }
  var b64data = "";
  var screenshooterJar = path.resolve(helperJarPath, 'ScreenShooter.jar');
    ...
    function(cb) {
      exec('uname -m', function (error, stdout, stderr) {
        if (error) {
          ...
          classpath = jarpath + "/ddmlib.jar:" + jarpath + "/" + stdout.trim() + "/swt.jar:" + screenshooterJar;
          cb(null);
        }
      });
    },
